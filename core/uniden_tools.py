...
Responsibilities:
  ...
  * Locate the bundled installer bootstrappers we keep inside the repo
    (``vendor/uniden_installers/BT885_UpdateManager_V0_00_05/`` etc.) so we can offer one-click
...
VENDOR_INSTALLERS_REL = Path("vendor") / "uniden_installers"


def _repo_root() -> Path:
    """Return the scanner-manager repository root (parent of ``core/``)."""
...
def _bundled_installer(
    ...
    *,
    ...
    Search order (first hit wins):
      ...
      2. A developer-local copy under ``vendor/uniden_installers/``
         (e.g. ``vendor/uniden_installers/BCDx36HP_Sentinel_Version_3_01_01/setup.exe``).
         Not shipped in release builds; dev/RE convenience for offline work.

...
| **Multi-scanner backend (residual)** | Local | Two profiles shipped. Qt auto profile switch on card load shipped. Legacy Tk `ACTIVE_PROFILE` still import-time only. | `scanner_profiles/`, `core/device_manager.py`, `gui/header.py`, `gui/editor/editor_dock.py` | Remaining: migrate legacy Tk globals, retire `compat.py`, Tk `detect_from_card`. |

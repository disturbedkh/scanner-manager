...
- [ ] You finished [`sentinel_capture.md`](sentinel_capture.md) -
      so `Metacache/Dev/RE/sentinel_pcaps/` exists with `.pcapng` files
...
```
# decoding Metacache\Dev\RE\sentinel_pcaps\01_read_from_scanner.pcapng
...
# decoding Metacache\Dev\RE\sentinel_pcaps\02_write_to_scanner.pcapng
...
```
...
1. Open the corresponding `.commands.jsonl` and grep:
   ...
   ```powershell
   Select-String -Path Metacache\Dev\RE\sentinel_pcaps\01_read_from_scanner.commands.jsonl `
...
4. Cross-reference against
   [`Metacache/Dev/RE/firmware_analysis/sub_1.03.15.strings.txt`](firmware_analysis/sub_1.03.15.strings.txt)
...
```powershell
py Metacache\Dev\RE\_correlate_responses.py
```

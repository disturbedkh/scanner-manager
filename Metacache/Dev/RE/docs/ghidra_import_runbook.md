...
2. Download `LPC43xx.svd` (or `LPC4350_4350.svd` if you want a
   ...
   `Metacache/Dev/RE/firmware/LPC43xx.svd`.
...
2. Name it `SDS100_SUB`. Save under
   `Metacache\Dev\RE\firmware\SDS100_SUB\` (Ghidra creates a folder for
   project files).
...
3. Browse to `Metacache\Dev\RE\firmware\LPC43xx.svd` -> OK.

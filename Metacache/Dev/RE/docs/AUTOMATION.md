...
├── _sentinel_session.py              # Layer 3: passive USBPcap capture driver
├── _analyze_ghidra_dump.py           # Consumer of analysis_dump.json
└── _verify_dispatch.py               # Live falsification probe (COM3)
```
...
3. **Bootstrap Ghidra:**
   ...
   ```powershell
   powershell -ExecutionPolicy Bypass -File Metacache\Dev\RE\automation\bootstrap_ghidra.ps1
   powershell -ExecutionPolicy Bypass -File Metacache\Dev\RE\automation\fetch_lpc43xx_svd.ps1
   ```
...
4. **Audit:**
   ...
   ```powershell
   powershell -ExecutionPolicy Bypass -File Metacache\Dev\RE\automation\check_prereqs.ps1
   ```
...
powershell -ExecutionPolicy Bypass -File Metacache\Dev\RE\automation\run_ghidra_setup.ps1

# 2. Consume the JSON. Produces sub_command_dispatch.md,
#    SDS100_inter_mcu_protocol.md, sessions/dispatch_candidates.txt,
#    and an auto-generated annotation block in sub_command_response_map.md.
py Metacache\Dev\RE\_analyze_ghidra_dump.py
```

To re-run with a freshly-edited Java post-script use `-Force`:

```powershell
powershell -ExecutionPolicy Bypass -File Metacache\Dev\RE\automation\run_ghidra_setup.ps1 -Force
```

To re-run only the dump (skip ~1 minute analyze-everything cycle):

```powershell
powershell -ExecutionPolicy Bypass -File Metacache\Dev\RE\automation\run_ghidra_setup.ps1 -DumpOnly
```
...
py Metacache\Dev\RE\_sentinel_session.py
...
py Metacache\Dev\RE\_sentinel_session.py --decode-only

# Skip operations you don't want this session:
py Metacache\Dev\RE\_sentinel_session.py --skip 4 --skip 6
```

### Falsification: confirm Ghidra's predictions on the live scanner

```powershell
# After _analyze_ghidra_dump.py has produced dispatch_candidates.txt:
py Metacache\Dev\RE\_verify_dispatch.py --port COM3
```
...
- USBPcap requires a reboot after install before the kernel driver
  starts capturing. `_sentinel_session.py` will report "no SDS100
  traffic seen" if it ran before the reboot.
- The dispatch-candidate verifier (`_verify_dispatch.py`) produces an

...
::
    ...
    py Metacache/Dev/RE/_decompile_pull.py

    # Specific functions (addresses or names, comma- or space-separated):
    py Metacache/Dev/RE/_decompile_pull.py 0x14010fec
    py Metacache/Dev/RE/_decompile_pull.py 0x14010554 0x1400e57c 0x1400eb24
    py Metacache/Dev/RE/_decompile_pull.py FUN_14010fec

    # Just list what's already been decompiled, don't re-run Ghidra:
    py Metacache/Dev/RE/_decompile_pull.py --list

    # Print one function's decompile to stdout (after a previous pull):
    py Metacache/Dev/RE/_decompile_pull.py --show 0x14010fec
...
4. Lists the resulting JSON files under
   ``Metacache/Dev/RE/firmware/decompiles/`` with summary stats:
     - addr / name / size
     - peripheral_accesses

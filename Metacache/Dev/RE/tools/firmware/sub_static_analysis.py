...
def _target_format_strings(payload: bytes) -> list[bytes]:
    target_strings = [
...
def _write_mnemonic_section(f, mnemonics: list, clusters: list) -> None:
...
def _write_peripheral_section(f, addrs: dict) -> None:
...
def _write_xref_section(f, string_xrefs: list) -> None:
...
def _write_static_report(
    ...
    *,
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        ...
        _write_mnemonic_section(f, mnemonics, clusters)
        _write_peripheral_section(f, addrs)
        _write_xref_section(f, string_xrefs)
...
def main() -> int:
    ...
    target_strings = _target_format_strings(payload)

...
.PARAMETER ProjectDir
  Where the Ghidra project lives. Default: Metacache\Dev\RE\firmware
...
if (-not $ProjectDir) { $ProjectDir = Join-Path $repoRoot 'Metacache\Dev\RE\firmware' }
if (-not $OutputDir)  { $OutputDir  = Join-Path $repoRoot 'Metacache\Dev\RE\firmware\decompiles' }

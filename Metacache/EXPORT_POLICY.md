...
The private **Forgejo** SSOT
([git.kjhuttoenterprises.com/disturbedkh/Scanner-Manager](https://git.kjhuttoenterprises.com/disturbedkh/Scanner-Manager))
tracks the **full** `Metacache/` tree (~240 files): RE lab notebooks,
probe sessions, firmware blobs, Ghidra projects, and agent handoff docs.
(GitLab.com is **deprecated** for this project.) The public GitHub mirror
ships a **filtered** subset so future RE contributors can reproduce our
work without machine-specific paths, raw USB captures, or internal agent
notebooks.
...
| Tier | GitHub | Private Forgejo | Examples (see YAML for full list) |
| --- | --- | --- | --- |
...
From repo root (after committing to Forgejo `gitea/main`):
...
1. Clones private remote `gitea` into a temp directory (never mutates your working tree).
...
Private HTTPS clone needs `GITEA_TOKEN` / `FORGEJO_TOKEN` (or a credential
helper). Large initial history pushes may need SSH via
`gitssh.kjhuttoenterprises.com` + `cloudflared` (see [`Dev/CURSOR.md`](Dev/CURSOR.md)).
...
`MACHINES.md` (if you maintain one locally) is private-only and stripped
on export.

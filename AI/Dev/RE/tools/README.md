...
> Companion docs:
> - `AI/Dev/RE/docs/` - canonical lab notebooks (BT885.md, SDS100.md,
...
> - `AI/Dev/RE/specs/` - vendor PDFs / TXTs of the official Uniden
...
`*_inflated.bin` in `AI/Dev/RE/firmware/`) so you can re-run the
...
py AI/Dev/RE/tools/probes/list_ports.py

# 2. Sweep MAIN with the documented Uniden Remote Command surface
py AI/Dev/RE/tools/probes/serial_probe.py

# 3. Sweep SUB with first-byte / two-byte / format-string probes
py AI/Dev/RE/tools/probes/sub_probe.py
```

### B. Reverse a new SUB firmware version

```powershell
# 1. Drop the new SDS-100-SUB_VX_YZ_AB.firm into AI/Dev/RE/firmware/

# 2. Inflate -> ARM payload + chunk-map markdown
py AI/Dev/RE/tools/firmware/inflate_sub.py

# 3. Extract strings (auto-diffs against the previous SUB version)
py AI/Dev/RE/tools/firmware/firmware_strings.py

# 4. Static analysis report
py AI/Dev/RE/tools/firmware/sub_static_analysis.py

# 5. (optional) Run Ghidra headless on the new payload
.\AI\Dev\RE\tools\automation\run_ghidra_setup.ps1
.\AI\Dev\RE\tools\automation\run_ghidra_decompile.ps1
py AI/Dev/RE/tools/firmware/analyze_ghidra_dump.py
```
...
py AI/Dev/RE/tools/sentinel/sentinel_session.py --decode

# Decode an existing pcap on its own:
py AI/Dev/RE/tools/sentinel/decode_sentinel_pcap.py path/to/01_read.pcap
```

"""Shared helpers for ``AI/Dev/RE/tools/`` scripts.
...
def find_latest_sub_firmware() -> Optional[Path]:
    ...
    ``AI/Dev/RE/firmware/``, or ``None`` if no inflated SUB firmware
...
def find_latest_sub_strings() -> Optional[Path]:
    ...
    ``AI/Dev/RE/firmware_analysis/``, or ``None`` if none exists.
    """
...
def add_firmware_arg(
    ...
    *,
    ...
    help_text: str = "Path to firmware blob to analyse. Default: most-recent inflated SUB blob in AI/Dev/RE/firmware/.",
) -> None:

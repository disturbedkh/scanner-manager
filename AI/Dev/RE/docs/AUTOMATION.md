...
3. **Bootstrap Ghidra:**
   ...
   ```powershell
   powershell -ExecutionPolicy Bypass -File AI\Dev\RE\automation\bootstrap_ghidra.ps1
   powershell -ExecutionPolicy Bypass -File AI\Dev\RE\automation\fetch_lpc43xx_svd.ps1
   ```
...
4. **Audit:**
   ...
   ```powershell
   powershell -ExecutionPolicy Bypass -File AI\Dev\RE\automation\check_prereqs.ps1
   ```
...
# AI/Dev/RE/firmware/sub_1.03.15_inflated.bin must exist.

# 1. Run Ghidra import + analysis + JSON dump.
powershell -ExecutionPolicy Bypass -File AI\Dev\RE\automation\run_ghidra_setup.ps1
...
py AI\Dev\RE\_analyze_ghidra_dump.py
```

To re-run with a freshly-edited Java post-script use `-Force`:

```powershell
powershell -ExecutionPolicy Bypass -File AI\Dev\RE\automation\run_ghidra_setup.ps1 -Force
```

To re-run only the dump (skip ~1 minute analyze-everything cycle):

```powershell
powershell -ExecutionPolicy Bypass -File AI\Dev\RE\automation\run_ghidra_setup.ps1 -DumpOnly
```
...
py AI\Dev\RE\_sentinel_session.py
...
py AI\Dev\RE\_sentinel_session.py --decode-only

# Skip operations you don't want this session:
py AI\Dev\RE\_sentinel_session.py --skip 4 --skip 6
```
...
py AI\Dev\RE\_verify_dispatch.py --port COM3
```

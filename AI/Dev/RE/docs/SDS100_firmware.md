...
> `<HOST>`. Tooling: `AI/Dev/RE/_firmware_strings.py`,
> `AI/Dev/RE/_firmware_structure.py`. Outputs in
...
4. **The SUB port is under-explored.** Session 4 only sent 10
   ...
   (`AI/Dev/RE/sub_probe.py`) and walk a short alphabet of
...
- `AI/Dev/RE/_firmware_strings.py` - extracts ASCII runs from each
...
- `AI/Dev/RE/_firmware_structure.py` - per-image entropy profile,
  hex dump of head/tail, magic-byte signature scan, byte-level diff
  between same-MCU versions.
- `AI/Dev/RE/firmware_analysis/firmware_structure_report.md` - the
  rendered structural report.
- `AI/Dev/RE/firmware_analysis/sub_1.03.15.strings.txt` etc. - raw

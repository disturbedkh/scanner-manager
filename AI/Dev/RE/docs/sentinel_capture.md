...
> `py AI\Dev\RE\_sentinel_session.py` and follow the prompts. It
...
`AI/Dev/RE/sentinel_pcaps/` will contain:
...
4. **Wireshark**: File -> Save As ->
   `<repo>\AI\Dev\RE\sentinel_pcaps\<NN_name>.pcapng`.

> Make the directory first:
> `mkdir AI\Dev\RE\sentinel_pcaps` (or use Explorer).
...
When all the pcaps are in `AI/Dev/RE/sentinel_pcaps/`, run:

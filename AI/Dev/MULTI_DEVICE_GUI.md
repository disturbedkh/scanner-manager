...
- `AI/Dev/MULTI_SCANNER_BACKEND.md` - the existing backend doc.
  This GUI plan implements its "GUI follow-up" deferred items.
- `AI/Dev/RE/SDS100.md` - what we know about the SDS100's serial
  command surface; informs the live panel's design.
- `AI/Dev/RE/BT885.md` - the BT885's surface; informs why some
  flags differ (no FLs, no waterfall, no serial mode).
- `AI/Dev/FIRMWARE_UPDATER.md` - the firmware updater's GUI
